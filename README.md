...
일반적으로 개발자는 동일한 코드 패턴을 반복적으로 작성하게 되는데, 이러한 코드를 체계적으로 관리하지 않으면 생산성이 저하된다. 이를 해결하기 위해 Snippet-Vault 시스템을 설계하였다.
...
| 설명 | 새로운 코드 스니펫을 저장한다 |
| 사전 조건 | 시스템이 실행 중 |
| 기본 흐름 | 1. 사용자가 스니펫 생성 화면 진입<br>2. 제목 입력<br>3. 코드 입력<br>4. 언어 선택<br>5. 태그 선택<br>6. 저장 |
| 결과 | DB에 스니펫 저장 |
...
| 행위자 | 사용자 |
| 설명 | 저장된 스니펫 목록 조회 |
| 기본 흐름 | 1. 홈 화면 접속<br>2. 스니펫 목록 표시 |
| 결과 | 스니펫 목록 확인 |
...
| 행위자 | 사용자 |
| 설명 | 기존 스니펫 수정 |
| 기본 흐름 | 1. 상세 화면 진입<br>2. 수정 클릭<br>3. 데이터 수정<br>4. 저장 |
| 결과 | DB 업데이트 |
...
- SnippetTag는 다대다 관계를 위한 중간 테이블이다.
...
홈 화면에서는 저장된 스니펫 목록을 확인할 수 있으며,
검색 기능과 언어 필터를 통해 원하는 스니펫을 빠르게 찾을 수 있다.
...
![화면 캡처 2026-04-07 004958](https://github.com/user-attachments/assets/cd5afefd-e7f2-4b69-9acd-4d2e16fbb728)
![화면 캡처 2026-04-07 005011](https://github.com/user-attachments/assets/1df4e043-3770-4d0b-90f1-c170734edcf8)
...
Description: React에서 데이터 fetch하는 useEffect 예제
...
response util 함수에서 HTTP 상태 코드 대신 문자열이 전달되어 오류 발생

### 해결 방법

statusCode와 message를 분리하여 전달하도록 수정
...
### 원인 
/snippets/create 경로가 /snippets/:id 라우트와 충돌하여 create 페이지가 id로 인식되는 문제 발생

### 해결 방법
라우트 정의 순서를 수정하여 create 경로가 먼저 인식되도록 변경

# 7. 결론

Snippet-Vault 시스템은 개발자가 자주 사용하는 코드 조각을 저장하고 관리할 수 있도록 설계된 웹 애플리케이션이다.
...
이를 통해 개발자가 필요한 코드를 빠르게 찾고 재사용할 수 있도록 하여 개발 생산성을 향상시키는 것을 목표로 구현하였다.
